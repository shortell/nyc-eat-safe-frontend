...
export default function NGradeExplanation({ className = "", collapsible = false }) {
        ...
        <div className={`bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden ${className}`}>
            ...
            <div className={`transition-all duration-300 ease-in-out overflow-hidden ${isOpen ? 'max-h-[500px] opacity-100' : 'max-h-0 opacity-0'}`}>
                <div className="px-6 pb-6 pt-0 text-slate-600 text-sm leading-relaxed space-y-4">
                    <p>
                        An <strong>N</strong> grade in most cases means that an inspection has happened, the inspector found violations and gave a score, but did not assign a letter grade.
                    </p>
                    <div className="bg-amber-50 border border-amber-100 rounded-lg p-4">
                        ...
                        <ul className="list-disc list-outside space-y-1 ml-5 text-amber-800">
                            ...
                            <li>Restaurants with an &quot;N&quot; and <strong>no score</strong> usually haven&apos;t been inspected at all.</li>
                        </ul>
                    </div>
                </div>
            </div>
